...
import ie.tus.project.services.AppContext;
...
private void analyticsMenu() {
	...
	do {
		...
		case 3 -> {
			...
			analytics.sortByName(characters).forEach(c -> System.out.println(summary(c)));
		}

		case 4 -> {
			System.out.println("\nCharacters sorted by level:");
			analytics.sortByLevel(characters).forEach(c -> System.out.println(summary(c)));
		}

		case 5 -> analytics.highestLevelCharacter(characters).ifPresentOrElse(
				c -> System.out.println("Highest level: " + summary(c)),
				() -> System.out.println("No character found."));

		case 6 -> analytics.lowestLevelCharacter(characters).ifPresentOrElse(
		...
		case 7 -> {
			...
			analytics.groupByClass(characters)
					.forEach((pClass, list) -> System.out.println(pClass + ": " + list.size() + " character(s)"));
		}

		case 8 -> {
			System.out.println("\nCharacters grouped by species:");
			analytics.groupBySpecies(characters)
					.forEach((species, list) -> System.out.println(species + ": " + list.size() + " character(s)"));
		}

		case 9 -> {
			System.out.println("\nPartitioned by level 5 or higher:");
			analytics.partitionByLevelFiveOrHigher(characters).forEach((levelFiveOrHigher, list) -> System.out.println(
					(levelFiveOrHigher ? "Level 5+" : "Below Level 5") + ": " + list.size() + " character(s)"));
		}

		case 10 -> System.out.println("Distinct classes: " + analytics.distinctClasses(characters));
		case 11 -> {
			System.out.println("\nFirst 3 characters:");
			analytics.firstThreeCharacters(characters).forEach(c -> System.out.println(summary(c)));
		}
		case 12 -> {
...
private String summary(PCharacter c) {
	return c.getName() + " | Class: " + c.getPClass() + " | Species: " + c.getSpecies() + " | Level: " + c.getLevel()
			+ " | HP: " + c.getHp();
}
...
private PCharacter editCharacter(PCharacter originalCharacter) {
	...
	do {
		...
		case 4 -> {
			newLevel = readInt("Enter new level (1–3):", 1, 3);
		}
		case 5 -> {
			originalCharacter = editStats(originalCharacter);
			newStats = originalCharacter.getStats();
		}
	...
	return new PCharacter(originalCharacter.name(), newClass, newSpecies, newBackground, newStats, newLevel,
			updatedAudit);
...
private PCharacter editStats(PCharacter pc) {
	...
	int newHp = Math.max(1,
			pc.pClass().hitDie() + (pc.getLevel() - 1) * (pc.pClass().hitDie() / 2 + 1) + conMod * pc.getLevel());
	AuditInfo updatedAudit = pc.getAuditInfo().update();
	// Return Edited Character
	return new PCharacter(pc.name(), pc.pClass(), pc.species(), pc.background(), stats, pc.getLevel(), updatedAudit);
}
...
public void createCharacter() {
	...
	// Add Character
	PCharacter playerCharacter = new PCharacter(name, pClass, species, background, abilityScores, 1,
			AuditInfo.createNew());
...
public void createCharacter(String name, Classes pClass, Species species) {
	...
	PCharacter playerCharacter = new PCharacter(name, pClass, species, background, abilityScores);
...
public int[] rollStatsWithReroll() {
	...
	do {
		...
		// Reroll
			...
			System.out.println("Total below <70. Rerolling...\n");
		}
...
public List<PCharacter> loadCharacters() {
	...
	try {
		...
		// If Characters Empty
		if (rawCharacters.equals("[]"))
			return characters;

		// If not Empty
		// Remove surronding brackets
		rawCharacters = rawCharacters.substring(1, rawCharacters.length() - 1).trim();
		// Split rawJson Characters into individual Objects based on Regex for
		// \\}\\s*,\\s*\\{
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (!jsonCharacter.startsWith("{"))
				jsonCharacter = "{" + jsonCharacter;
			if (!jsonCharacter.endsWith("}"))
				jsonCharacter = jsonCharacter + "}";
			...
			// Get Stats and removes [], split on ',', remove Whitespacem convert String to
			// Int and convertes to Array
...
/*
 * Helper Method Selection logic Loops until input is between min & max
 */
...
private void changeLanguage() {
	...
	ScopedValue.where(AppContext.CURRENT_LOCALE, localisation.currentLocale()).run(() -> {

		System.out.println("Scoped locale active: " + AppContext.CURRENT_LOCALE.get());

		System.out.println(localisation.text("message.languageChanged"));
	});
}

private void scopedValueDemo() {
	ScopedValue.where(AppContext.CURRENT_LOCALE, localisation.currentLocale()).run(() -> {
		System.out.println("\n--- Scoped Value Demo ---");
		System.out.println("Current scoped locale: " + AppContext.CURRENT_LOCALE.get());

		System.out.println("Current language: "
				+ AppContext.CURRENT_LOCALE.get().getDisplayLanguage(AppContext.CURRENT_LOCALE.get()));

		System.out.println("This locale value is only available inside this scoped block.");
	});
}
