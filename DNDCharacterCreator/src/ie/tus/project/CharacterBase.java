...
public non-sealed abstract  class CharacterBase implements Describable {
    ...
    protected CharacterBase(String name, int level, int[] stats) {
        ...
        if (stats == null || stats.length != 6) {throw new IllegalArgumentException("Stats must be an array of 6 values");}
    ...
    @Override
    public String description() {return name + " is level " + level + " with HP " + hp;}
}
