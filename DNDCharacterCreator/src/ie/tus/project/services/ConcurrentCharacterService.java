...
public class ConcurrentCharacterService {
	...
	public List<String> calculateStatSummaries(List<PCharacter> characters) throws InterruptedException {
	...
	private String buildStatSummary(PCharacter character) {
		...
		int totalModifiers = Arrays.stream(stats).map(PCharacter::abilityModifier).sum();

		return character.getName() + " | Class: " + character.getPClass() + " | Level: " + character.getLevel()
				+ " | Total Stats: " + totalStats + " | Total Modifiers: " + totalModifiers;
	}
}