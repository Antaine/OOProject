...
public class CharacterAnalyticsService {
	...
	public List<PCharacter> sortByName(List<PCharacter> characters) {
		return characters.stream().sorted(Comparator.comparing(PCharacter::getName)).toList();
	}

	public List<PCharacter> sortByLevel(List<PCharacter> characters) {
		return characters.stream().sorted(Comparator.comparing(PCharacter::getLevel).reversed()).toList();
	}

	public List<PCharacter> sortByClass(List<PCharacter> characters) {
		return characters.stream().sorted(Comparator.comparing(c -> c.getPClass().name())).toList();
	}

	public List<PCharacter> sortBySpecies(List<PCharacter> characters) {
		return characters.stream().sorted(Comparator.comparing(c -> c.getSpecies().name())).toList();
	}

	public List<PCharacter> sortByLastEdited(List<PCharacter> characters) {
		return characters.stream().sorted(Comparator.comparing(c -> c.getAuditInfo().lastEdited())).toList();
	}
	...
	public List<PCharacter> filterByClass(List<PCharacter> characters, Classes pClass) {
		...
		return characters.stream().filter(sameClass).toList();
	}
	...
	public List<PCharacter> filterBySpecies(List<PCharacter> characters, Species species) {
		...
		return characters.stream().filter(sameSpecies).toList();
	}
	...
	public List<String> characterNames(List<PCharacter> characters) {
		...
		return characters.stream().map(nameFunction).collect(Collectors.toList());
	}

	public List<Classes> distinctClasses(List<PCharacter> characters) {
		return characters.stream().map(PCharacter::getPClass).distinct().toList();
	}

	public List<PCharacter> firstThreeCharacters(List<PCharacter> characters) {
		return characters.stream().limit(3).toList();
	}

	// Collectors
	public Map<Classes, List<PCharacter>> groupByClass(List<PCharacter> characters) {
		return characters.stream().collect(Collectors.groupingBy(PCharacter::getPClass));
	}

	public Map<Species, List<PCharacter>> groupBySpecies(List<PCharacter> characters) {
		return characters.stream().collect(Collectors.groupingBy(PCharacter::getSpecies));
	}

	public Map<Boolean, List<PCharacter>> partitionByLevelFiveOrHigher(List<PCharacter> characters) {
		return characters.stream().collect(Collectors.partitioningBy(c -> c.getLevel() >= 5));
	}

	public Map<String, PCharacter> mapNameToCharacter(List<PCharacter> characters) {
		return characters.stream()
				.collect(Collectors.toMap(PCharacter::getName, Function.identity(), (first, duplicate) -> first));
	}

	public Map<String, Integer> mapNameToLevel(List<PCharacter> characters) {
		return characters.stream()
				.collect(Collectors.toMap(PCharacter::getName, PCharacter::getLevel, (first, duplicate) -> first));
	}

	// Supplier example
	public PCharacter createDefaultAnalyticsCharacter() {
		Supplier<PCharacter> defaultCharacter = () -> new PCharacter("Analytics Test Character", Classes.Fighter,
				Species.HUMAN);
	...
	private boolean hasValidStats(PCharacter character) {
		...
		return stats.length == 6 && java.util.Arrays.stream(stats).allMatch(stat -> stat >= 3 && stat <= 20);
	}

	public List<List<PCharacter>> characterPages(List<PCharacter> characters, int pageSize) {
		return characters.stream().sorted(Comparator.comparing(PCharacter::getName))
				.gather(Gatherers.windowFixed(pageSize)).toList();
	}
}